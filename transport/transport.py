...
class Transport:
    ...
    def __init__(
        ...
        *,
        ...
        self.law = logitnorm(ln_mu, ln_sigma)  
    ...
    def sample(self, x1):
        ...
        ### This part code will be released after accept ###
        # if  self.path_type == PathType.VP:
        #     simulated = VR_simulated_pdf_diff()
        # elif self.path_type == PathType.GVP:
        #     simulated = VR_simulated_pdf()
        # elif self.path_type == PathType.LINEAR:
        #     simulated = VR_simulated_pdf_linear()

        # if self.log_norm == False and self.vr_sampling == False:
        #     t = th.rand((x1.shape[0],)) * (t1 - t0) + t0
        #     t = t.to(x1)
        if self.log_norm == False:
            t = th.rand((x1.shape[0],)) * (t1 - t0) + t0
            t = t.to(x1)
        elif self.log_norm == True:
            t = self.law.rvs(size=x0.shape[0])
            t = t * (t1 - t0) + t0
            t = th.from_numpy(t).type_as(x1)
        # elif self.vr_sampling == True:
        #     t = simulated.sample(x0.shape[0])
        #     t = th.from_numpy(t).type_as(x1)
